...
uint8_t DHT11_ReadBit(void) {
    ...
    return (timeCount > 40) ? 1 : 0; // Return 1 if HIGH duration > ~40탎
}
...
// Function to read a single bit from the DHT11
 /*uint8_t DHT11_ReadBit(void) {
    ...
    if (timeCount > 40) { // If HIGH duration > ~30탎, it's a '1'
      //
      dio_writepin('F', 2, 1);
    /*LED_Delay(2500000);
     dio_writepin('F', 1, 0);*/
...
uint8_t i, j;
    ...
    // Wait for sensor's response signal
    if (!WaitForPinState(0, 10000)) return -1; // Wait for LOW (80탎)
    if (!WaitForPinState(1, 10000)) return -1; // Wait for HIGH (80탎)
